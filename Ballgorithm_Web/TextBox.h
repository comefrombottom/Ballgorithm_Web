...
inline void UpdateText(String& text, int32& cursorPos, int32& selectEndPos, const TextInputMode mode)
{
...
public:
	...
	void selectAll()
	{
		caretIndex = static_cast<int32>(text.size());
		selectionEndIndex = 0;
	}
...
# endif
		...
		if (not editingGlyphs and isFocused) {
			...
			if (keyControlOrCommandPressed and KeyV.down())
			{
				...
				if (Clipboard::GetText(clipText))
				{
					...
					caretIndex += static_cast<int32>(clipText.size());
					selectionEndIndex = caretIndex;
				}
			}
		}
		...
		if (not editingGlyphs and isFocused) {
			...
			caretIndex = Clamp<int32>(caretIndex, 0, static_cast<int32>(glyphs.size()));
			selectionEndIndex = Clamp<int32>(selectionEndIndex, 0, static_cast<int32>(glyphs.size()));
		...
		if (not editingGlyphs and isFocused) {
			...
			if (MouseL.down())
			{
				...
				for (auto [i, glyph] : Indexed(glyphs)) {
					...
					{
						selectionEndIndex = static_cast<int32>(i);
						break;
					}
					pos.x += xAdvance;
					if (i + 1 == glyphs.size())
					{
						selectionEndIndex = static_cast<int32>(glyphs.size());
					}
				}
			}
			...
			if (MouseL.pressed())
			{
				...
				for (auto [i, glyph] : Indexed(glyphs)) {
					...
					if (Cursor::Pos().x < pos.x + xAdvance / 2)
					{
						caretIndex = static_cast<int32>(i);
						break;
					}
					pos.x += xAdvance;
					if (i + 1 == glyphs.size())
		...
		if (isFocused and not editingText and caretIndex == previousCaretIndex and caretIndex == selectionEndIndex)
		{
			timeAfterCaretStopped += deltaTime;
		}
		else
		{
			timeAfterCaretStopped = 0.0;
		}
		...
		if (not editingText) {
			...
			if (previousCaretIndex != caretIndex)
			{
				Platform::Web::TextInput::SyncronizeText(text);
		...
		double allTextLastX = 0.0;
		{
			...
			for (auto [g_i, glyph] : Indexed(glyphs)) {
				...
				if (g_i + 1 == caretIndex)
				{
					editingStartX = posX;
					caretPosX = posX;
					for (const auto [e_i, editingGlyph] : Indexed(editingGlyphs))
					{

						posX += editingGlyph.xAdvance * scale;

						if (e_i + 1 == caretIndexInIME)
						{
							caretPosX = posX;
						}

						if (e_i + 1 == caretIndexInIME + imeTextCursorLength)
						{
							phraseUnderlineEndX = posX;
						}
					}
			...
			if (isFocused) {
				...
				if (caretIndex != selectionEndIndex)
				{
					...
					if (drawPhraseUnderline)
					{
						...
						constexpr double timeBeforeBlinking = 1.5;
						const double blinkInterval = 0.5;
						const bool caretVisible = (timeAfterCaretStopped < timeBeforeBlinking) || (static_cast<int32>((timeAfterCaretStopped - timeBeforeBlinking) / blinkInterval) % 2 != 0);
...
struct TextBox {
	...
	void draw() {
		const ColorF baseColor{ 0.12, 0.16, 0.22, 0.95 };
		const ColorF hoverColor{ 0.18, 0.24, 0.35, 0.95 };
		const ColorF textColor{ 0.95, 0.97, 1.0 };
		const ColorF selectionColor{ 0.25, 0.5, 0.8, 0.35 };
		body.draw(editableText.isFocused ? hoverColor : baseColor);
		if (editableText.isFocused) {
			...
			body.draw(baseColor.lerp(hoverColor, mouseOverTransition));
			body.drawFrame(2, ColorF(0.35, 0.45, 0.6, 0.6));
		}
		editableText.draw(textColor, selectionColor);
		if (not editableText.hasTextOrEditingText())
		{
			editableText.font(U" Enter text...").draw(editableText.fontSize, editableText.topLeft, ColorF(0.6, 0.7, 0.8, 0.7));
		}
	}
};
